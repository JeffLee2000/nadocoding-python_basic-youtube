...
from tkinter import filedialog # 서브 모듈이므로 __all__에 포함되지 않음
...
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요", \
        ...
        initialdir=r"C:\Users\lnb20\OneDrive\바탕 화면\이태호\Programming\vscode-wrokspace\nadocoding-python_basic-youtube")
...
def merge_image():
    ...
    try:
        ...
        # 계산식
        # 100 * 60 이미지가 있음. -> widthㄹ르 80으로 줄이면 height 는?
        # 원본 widht : 원본 height = 변경된 width : 변경된 heigth
        #    100     :     60     =      80      :       ?
        #     x      :     y      =      x'      :       y'
        #  xy' = x'y
        # y' = x'y / x -> 이 식을 적용
        ...
        for idx, img in enumerate(images):
            # width 가 원본유기가 아닐 때에는 이미지 크기 조정
            if img_width > -1:
...
file_frame.pack(fill="x", padx=5, pady=5) # padx, pady: 간격 띄우기
...
entry_dest_path.pack(side="left", fill="x", expand=True, padx=5, pady=5, ipady=4) # ipady: 높이 변경
