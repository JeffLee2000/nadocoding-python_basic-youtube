...
# 공 크기에 따른 최초 스피드
ball_speed_y = [-18, -15, -12, -9] # index 0, 1, 3, 4에 해당하는 값
...
while running:
    ...
    for event in pygame.event.get():    
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_LEFT:    # 캐릭터를 왼쪽으로
                ...
                weapon_y_pos = character_y_pos = character_y_pos
    ...
    for weapon_xpos, weapon_y_pos in weapons:
        screen.blit(weapon, (weapon_xpos, weapon_y_pos))
        
    for idx, val in enumerate(balls):
        ball_pos_x = val["pos_x"]
        ball_pos_y = val["pos_y"]
        blal_img_idx = val["img_idx"]
        screen.blit(ball_images[blal_img_idx], (ball_pos_x, ball_pos_y))
